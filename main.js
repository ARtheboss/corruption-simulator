...
function run(){
	if(on == 0){
		...
		if(engagedInCorruption()){
			giveQuestion("Your driving licensce recently elapsed, and although you can't yet afford a car, you want to get it renewed so that you can drive your parent's car. When you go to the office, the man working there says it will take at least 5 months, but you can get it within a week if you pay him a small fee...",["Pay the fee (-$200)","Wait 5 months"]);
			on = 104;
		}else{
			giveQuestion("Your driving licensce recently elapsed, and although you can't yet afford a car, you still want to drive your parent's car.",["Renew licensce","Wait until you get a car"]);
			on = 104.5;
		}
	}else if(on == 104){
		if(results[104] == 1){
			money -= 200;
			giveInfo("You got your driving licensce, but it cost you a lot.");
		}else{
		...
		if(results[104] == 1){
			...
			if(engagedInCorruption()){
				giveQuestion("Even though you don't have a driving licensce, you still want to drive your parent's car to places.",["Drive their car","Abide the law"]);
			}else{
